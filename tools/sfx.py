...
def shot_beam(rng):
    """Лазер: две расстроенные пилы с вибрато — луч держится ровно столько, сколько рисует трассер."""
    n = S.n_of(0.16)
    vib = 25.0 * S.osc(30.0, n)
    a = S.osc(1400 + vib, n, "saw")
    b = S.osc(2100 + vib * 1.5, n, "saw") * 0.5
    x = (a + b) * S.env(n, 0.008, 0.06, hold=0.085)
    return S.drive(S.band(x, 900, 4200), 1.6)
...
def shot_ion(rng):
    """Ионка: кольцевая модуляция — трескучий разряд, который слышно как «не совсем пушку»."""
    n = S.n_of(0.24)
    x = S.osc(220.0, n, "saw") * S.osc(37.0, n)
    crackle = S.noise(n, rng) * (rng.random(n) > 0.86) * 0.4
    x = S.biquad(S.mix(x, crackle), 380, "hp") * S.env(n, 0.004, 0.23, power=2.0)
    # Верх у разряда режется: без этого треск шипит поверх всего боя и не даёт расслышать попадания.
    return S.biquad(S.drive(x, 2.2), 6200, "lp")
