...
/// <summary>
/// Баланс из shared/ (<see cref="Balance.Files"/>: корпуса, пушки, правила боя, NPC, лут, метеориты, магазин, галактика, задания, модули) с горячей перезагрузкой: файлы правят во время плейтеста,
...
public sealed class BalanceStore : IDisposable
{
    ...
    private static bool Parse(string[] texts, out Balance? balance, out string? error) =>
        Balance.TryParse(new BalanceSources(texts[0], texts[1], texts[2], texts[3], texts[4], texts[5], texts[6], texts[7], texts[8], texts[9]), out balance, out error);
