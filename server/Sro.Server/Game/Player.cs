    ...
    : ShipEntity(id, name, hullId, [weaponId])
{
    ...
    public override double Block(Balance balance, string damageType) => Fitting.Block(Fit, balance.Modules, damageType);
    ...
    /// <summary>
    /// Корабль в доке: его нет в космосе — ни в снапшоте, ни среди целей, ни на пути метеоритов.
    /// Пилот в это время торгует и меняет корабль.
    /// </summary>
    public bool Docked;

    /// <summary>
    /// Ключ места, где корабль стоит (M15): «st:vega» — станция, «pl:terra» — поселение. null — в космосе,
    /// а у профиля старше M15 — станция своей системы (её подставляет комната при входе).
    /// По этому ключу идут витрина, рынок, доска заданий и репутация места: их в системе стало несколько.
    /// </summary>
    public string? DockedPlace;

    /// <summary>
    /// Где корабль стыковался — в осях места (<see cref="OrbitDef.ToLocal"/>): место за это время ушло по орбите,
    /// а вылет — с той же его стороны.
    /// </summary>
    ...
    /// <summary>
    /// Место последней стыковки (M15): у него корабль и появится. null — место неизвестно (профиль старше M15
    /// или пилот ещё нигде не стоял), тогда берётся главное место домашней системы.
    /// </summary>
