...
internal sealed class MeteorSystem(Func<int> nextId, Random rng)
{
    ...
    private static void Ram(Meteor meteor, ShipEntity ship, Balance balance, long tick, List<ShotDto> shots)
    {
        ...
        meteor.Hp = 0;
        meteor.Rammed = true;
        // Камень, разбившийся о корабль пилота, засчитывается ему как уничтоженный (M16a): дальше его
        // подберёт общий список уничтоженных этого тика, и «охота» получит свой плюс — даже если пилот
        // погиб в том же столкновении. Второй раз засчитать нельзя: убийца пишется сюда один раз,
        // и выстрел его уже не перепишет.
        if (ship is Player && meteor.KilledBy == 0) meteor.KilledBy = ship.Id;
    }
