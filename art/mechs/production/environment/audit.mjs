...
for(const item of manifest.entries.filter(x=>x.source?.startsWith('sources/'))){
  try{
    ...
    for(let n=0;n<columns*rows;n++){
      ...
      frames.push({direction:item.variants[n]??n,rect:[left,top,right-left,bottom-top],visiblePixels:visible,transparentPixels:transparent,bounds:visible?[minX-left,minY-top,maxX-left,maxY-top]:null,minMargin:visible?Math.min(minX-left,minY-top,right-1-maxX,bottom-1-maxY):null});
    }
