...
| `desert` | Пыльная Гавань, Рудник Прайм | Пыльная Гавань | `desert-prime-*` для Рудника Прайм |

Поправка 2026-09-26: станций у Тау, Сигмы и Края больше нет (там только поселения), поэтому кадры
`tau-*`, `sigma-*` и `edge-*` из заказа убраны. Вместо них — пара для Рудника Прайм: он делит
пустынный набор с Пыльной Гаванью, и это единственная повторяющаяся пара, не попавшая в первый заказ.
...
- **desert-prime-office** (с `desert-office`) — диспетчер Рудника Прайм: славянин ~60, седые усы,
  потёртая куртка горняка с нашивкой рудника, респиратор на шее; за окном те же дюны, но копёр
  шахты и рудовозы. Не путать с Евой Морен (портрет пачки M) — она инженер этого же рудника.
- **desert-prime-trader** (с `desert-trader`) — торговка Рудника Прайм: восточноазиатка ~40,
  короткие волосы под банданой, защитные очки на лбу; на прилавке ящики с рудой и запчасти буров.

Итого 8 кадров. Подключение: файл → `client/public/dock/<набор>-<сцена>.webp` → имя набора в
...
## Пачка M — лица «Тихой войны» (после M20a)

Кампания вышла **без портретов**: карточка диалога (`client/src/ui/dialog.ts`) рисует монограмму —
две буквы имени в рамке. Слот под картинку в разметке уже есть (`.dialog-face`), подключение — одна
строка. Значок кампании на доске тоже нарисован кодом: золотая кромка у строки.

Портреты **256×256**, прозрачный фон не нужен — кадр по плечи, как паспортная карточка станции.
Стиль: STYLE-SCENE, но без интерьера: ровный тёмный фон «Стекла и стали», свет сбоку, лицо крупно.
Люди рабочие и уставшие, без героических póz: космос здесь — рабочее место.

- **portrait-eva** — Ева Морен, инженер Рудника Прайм: женщина ~35, короткая стрижка, рабочая роба
  с закатанными рукавами, на шее наушники-гарнитура, на скуле пыль. Смотрит прямо, без улыбки.
  Она единственная, кого игрок видит чаще всех, — на ней должна держаться вся кампания.
- **portrait-holt** — капитан Холт, начальник охраны Военной станции Нова: мужчина ~50, коротко
  стриженный, форменный китель без наград, тяжёлый взгляд. Не злодей: человек, выполняющий приказ.
- **portrait-dan** — Дан, пилот сопротивления (нужен в M20b): парень ~25, лётная куртка не по размеру,
  шарф, нервная полуулыбка. Единственное живое лицо в пачке.
- **portrait-contact** — безымянный контакт на станции (M20b): лицо наполовину в тени, поднятый
  воротник, ничего запоминающегося. Специально никакой.

Итого 4 кадра. **Сделано и подключено 2026-09-26**, но иначе, чем предлагалось: портреты режет
`tools/portraits.py` в `client/public/portraits/<имя>.webp` (не в спрайты — иначе их качал бы каждый
вход в игру), а карточка диалога находит портрет по имени говорящего (`PORTRAITS` в `dialog.ts`),
без нового поля в протоколе. Дан говорит сам, отдавая «Иглу» в финале; контакту реплик пока нет.

Позже, отдельным заходом: свои спрайты кораблей `rebel` (сейчас — «Тягач») и `corpGuard` (в M20b).
