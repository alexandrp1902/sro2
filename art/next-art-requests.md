...
Итого 4 кадра. Подключение: нарезать `tools/sprites.py` в `client/public/sprites/portrait-<имя>.webp`,
завести ключи в `client/src/render/spriteMeta.json`, добавить поле портрета в `DialogMsg`
(`server/Sro.Server/Net/Protocol.cs`) и заполнить его из `story.json` — у миссии уже есть `giver`,
рядом встанет `portrait`.
