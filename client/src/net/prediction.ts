...
export class Prediction {
  ...
  /**
   * Корабль перелетел в другую систему по тому же соединению: первый снапшот принимается как есть, но нумерация
   * входов продолжается. Начни она с 1, сервер отбросил бы новые входы как устаревшие: входы, отправленные
   * ещё до прыжка, доходят уже в новую систему, и сервер запоминает их номер.
   */
  resync(): void {
    this.pending = [];
    this.synced = false;
