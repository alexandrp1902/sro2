...
export interface HullParams {
  ...
  role?: string | null;
  /** Особенность корпуса (M19); нет — корпус без особенностей. */
  perk?: HullPerk | null;
}

/** Зеркало HullPerk на сервере (M19): то, что корпус умеет сверх чисел. */
export interface HullPerk {
  /** Множитель радиуса захвата груза; нет — обычный. */
  grab?: number;
  /** С какого расстояния видно контейнеры и обломки помимо радара; нет — только радаром. */
  scan?: number;
  /** Столкновение с камнем не бьёт по этому корпусу. */
  ram?: boolean;
}
