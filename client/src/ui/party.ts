...
/** Строка панели: ник, где он, полоски корпуса и щита. */
class RowView {
  ...
  set(row: PartyRow): void {
    const name = row.leader ? `★ ${row.name}` : row.name;
...
export class PartyPanel {
  ...
  /** null или пусто — прятать. */
  update(rows: PartyRow[] | null): void {
    ...
    rows.forEach((row, i) => this.views[i].set(row));
  }
}
