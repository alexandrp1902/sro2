 ...
 *
 * Картинок снижения нарисована пока одна, земная. Для остальных видов планет анимация идёт без кадра —
...
/** Виды планет, для которых нарисован кадр снижения. Остальные садятся «вслепую», через затемнение. */
const DRAWN = new Set(['terran']);
...
export function landingArt(kind: string | null | undefined): string | null {
  return kind && DRAWN.has(kind) ? `dock/landing-${kind}.webp` : null;
}
